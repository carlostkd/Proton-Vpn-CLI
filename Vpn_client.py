...
COLORS = {
    ...
    'error': '#ef4444'
def build_full_url():
    """Assemble the final URL from the selected parts."""
...
def run_curl(url: str):
    """Execute curl with the supplied URL and a custom User-Agent."""
...
def update_url_display(*args):
    """Update the URL display whenever inputs change."""
    url = build_full_url()
...
def show_documentation():
    """Display API documentation in the response window."""
    docs = """
 API DOCUMENTATION
...
def send_request():
    """Callback for the Send Request button."""
    url = build_full_url()
 ...
 Sending request...\n")
    ...
    def worker():
        ...
        response_text.config(state='disabled')
        send_btn.config(state='normal', text="
 Send Request")
...
token_entry = ttk.Entry(inner_frame, width=40, show="
", font=('Consolas', 10))
...
# send and documentation 
button_frame = tk.Frame(inner_frame, bg=COLORS['card'])
button_frame.grid(column=0, row=7, pady=(0, 20))
send_btn = tk.Button(button_frame, text="
 Send Request", bg=COLORS['accent'], fg=COLORS['fg'],
...
docs_btn = tk.Button(button_frame, text="
 API Documentation", bg=COLORS['input_bg'], fg=COLORS['fg'],
                     font=('Segoe UI', 11, 'bold'), relief=tk.FLAT, padx=30, pady=12,
